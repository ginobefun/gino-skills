...
Description 是模型判断是否加载 skill 的唯一依据。必须以 `Use when` 开头，描述**触发条件**而非功能清单。

**编写原则**:
- 回答"什么情况下应该加载这个 skill"
- 覆盖中英文用户的常见表述
- 当与 sibling skill 易混淆时，明确边界（如 `Use sibling skill Z instead for A`）
...
**禁止**:
- 不要在 description 中罗列编号式使用场景 `(1)... (2)...`
- 不要在 description 中放触发短语清单
- 不要描述功能列表或工作流摘要
...
1. **YAML Frontmatter**: `name` + `description`（以 `Use when` 开头，描述触发条件）
...
- **禁止** description 不以 `Use when` 开头或罗列功能/触发短语
...
- description 以 `Use when` 开头，覆盖中英文用户的常见表述（详见 `docs/skill-style-guide.md`）
