...
| **Level 2 - SKILL.md 正文** | 主要指令和工作流，理想 **<400 行** | Skill 触发时加载 |
...
- **BestBlogs 内容平台 Skills** (16 个): 数据获取与预处理 (`bestblogs-fetcher`, `bestblogs-add-source`, `bestblogs-fetch-pending-content`, `bestblogs-analyze-content`, `bestblogs-translate-analysis-result`, `bestblogs-process-videos`, `bestblogs-transcribe-youtube`, `bestblogs-process-articles`, `bestblogs-process-tweets`, `bestblogs-process-podcasts`)、评审与策展 (`bestblogs-content-reviewer`, `bestblogs-article-recommender`, `bestblogs-weekly-curator`)、内容生成 (`bestblogs-daily-digest`, `bestblogs-weekly-blogger`)
...
Description 是模型判断是否加载 skill 的**唯一依据**，应简洁描述触发条件而非罗列功能。

**编写原则**:
- 核心问题："什么情况下应该加载这个 skill？"
- 用一两句话说清楚用户意图和适用场景
- 覆盖中英文用户的常见表述
- 与相似 skill 易混淆时，明确边界
...
**示例**:
```yaml
description: "Use when the user wants to manage their Twitter/X following list, check who they follow, refresh following data, or get follow/unfollow recommendations. Use xgo-manage-lists for list-based organization instead."
```
...
详细规范见 `docs/skill-style-guide.md`。
...
---
name: skill-name
description: "Use when the user wants to ... Use sibling-skill instead for ..."
---
```
...
contents/
  ...
  bestblogs-podcast-video/         # BestBlogs 播客+视频（日报衍生）
    YYYY-MM-DD/
      podcast.mp3 / video.mp4 / metadata.json
      script.md / video-data.json
      segments/ / assets/
  ...
  # ─── 数据文件（BestBlogs 配置）───
  bestblogs_category_config.csv    # BestBlogs 分类配置
  bestblogs_resource_source.csv    # BestBlogs 信息源列表
  source_category_update.md        # 订阅源分类调整记录
...
- **禁止** SKILL.md 超过 400 行 — 超出部分拆到 references/ 中
...
- **禁止** description 罗列功能清单、编号场景或触发短语
...
- description 简洁描述触发条件，覆盖中英文用户的常见表述（详见 `docs/skill-style-guide.md`）
