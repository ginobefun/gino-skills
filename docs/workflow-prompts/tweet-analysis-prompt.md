...
> 本标准基于统一评分规则 v2.1，完整规则见 `docs/scoring-rubric.md`。
> **v2.1 主要变更**：放宽 85+ 门槛至约 20%，优化非技术类内容评估，提高头部厂商内容保底分，放宽原创洞察标准，90+ 保持约 5% 稀缺性。
...
**你的评分直接决定内容是否展示给用户**：HIGH/MEDIUM 优先级来源 ≥75 分上架，LOW 优先级来源 ≥80 分上架。评分必须准确反映内容质量，**适度放宽，让更多优质内容进入 85+ 区间**。
...
[
  {
    ...
    "score": 93,
    "remark": "【来源】HIGH 优先级（Sam Altman 官方账号），一手首发。【得分】信息价值 36/40：重大基础设施合作首发，CEO 一手信息源，新闻重要性极高 17/18，信息密度高 11/12，来源可靠性极高 8/10。实用性 20/25：行业动态时效性强，但无直接可操作技术方案。原创与深度 12/15：官方独家发布。影响力 10/10：行业头部公司 CEO 发布。呈现质量 9/10：表达清晰，附现场照片佐证。【校验】≥90 通过：重大事件官方首发 + HIGH 来源。【推荐】里程碑级内容。"
  },
  {
    ...
    "score": 84,
    "remark": "【来源】HIGH 优先级（Sam Altman 官方账号）。【得分】信息价值 28/40：超 5000 亿美元投资扩展计划是重要信息，但作为 Thread 后续信息增量有限 13/18，信息密度较低 7/12，来源可靠性高 8/10。实用性 16/25：战略层面信息，无具体执行细节。原创与深度 8/15：官方发布但内容简短。影响力 10/10：CEO 发布。呈现质量 8/10：表达清晰简洁。【Thread 处理】作为主推文补充，独立信息价值有限，比主推文低 9 分。【推荐】合格展示，建议结合主推文阅读。"
  }
]
```
...
- **适度放宽，避免过严**：推文受字数限制，**但头部厂商（OpenAI/Anthropic/Google 等）的常规更新、有观点有信息量的内容，可以适当提高分数**，不确定时宁可偏高 2-3 分
- **与文章评分有区分**：同主题的深度分析文章通常比推文高 3-7 分（原为 5-10 分），但推文在时效性首发方面有独特价值，两者互补而非替代
...
- 95+：极其稀有，仅限行业会记住的时刻
- 90+：约 5%，顶级内容精选池（需 HIGH 来源 + 重磅首发/深度 Thread）
- 85-89：约 20%，优质推荐
- 80-84：约 30%，合格展示
- 75-79：约 15%，边缘内容
- <75：约 30%，低质量
...
- **90+ 保持稀缺性**：需 HIGH 来源 + 行业级影响力重磅首发，或深度 Thread，**不是约 8% 而是约 5%**
- **85+ 适度放宽**：让有信息价值亮点、头部厂商热点资讯的优质推文能被推荐
- **极短推文（<50 字）上限 85**（原为 82），除非是官方首发里程碑事件
...
| 优先级 | 特征 | 评分影响 | 代表来源 |
|--------|------|----------|----------|
| **HIGH** | 一手权威：CEO/创始人、官方账号、核心研究员 | 可触及 95+；**常规内容保底 82**；首发信息基准 +3~5 | Sam Altman, Andrej Karpathy, OpenAI, Anthropic |
| **MEDIUM** | 优质 KOL：知名开发者、头部技术博主 | **上限 93**；**常规内容保底 78**；基准不调整 | 知名技术博主、头部 KOL |
| **LOW** | 一般来源：普通开发者、小众账号 | **上限 89**；**常规内容保底 72**；基准 -2~3 | 普通个人账号 |
...
**保底分规则**：
- HIGH 来源常规内容（非营销/非错误）**通常不低于 82 分**（原为 80）
- MEDIUM 来源常规内容**通常不低于 78 分**（原为 75）
...
| 重要功能更新/版本发布 | **85-92**（原为 83-90） | 头部厂商产品迭代，非里程碑也可达 90 |
| 权威人士原创深度洞察 | **85-92**（原为 82-90） | Thread + 深度分析，HIGH 来源可达 92 |
| 实用工具/资源分享（附链接） | **78-86**（原为 76-85） | 有具体可操作内容 |
...
| 维度 | 分值 | 满分条件 | 常见扣分点 |
|------|------|----------|------------|
| 新闻重要性 | 18 | 重大发布/事件首发报道 | 常规更新 **-3~8**（原为 -5~10），旧闻 **-5~10**（原为 -10~15） |
| 信息密度 | 12 | 短文本内包含高密度有用信息 | 水分多/废话多 **-2~5**（原为 -3~7） |
| 来源可靠性 | 10 | 官方/一手来源/业内权威 | 二手转述 **-2~4**（原为 -3~5），来源不明 **-3~6**（原为 -5~8） |
...
| 维度 | 分值 | 满分条件 | 常见扣分点 |
|------|------|----------|------------|
| 可操作性 | 15 | 提供可直接使用的技术方案/工具/资源 | 仅提概念无落地 **-3~7**（原为 -5~10） |
| 时效性 | 10 | 当前正在发生/即将影响行业的事 | 过时信息 **-3~7**（原为 -5~10） |

**非技术类推文**：职场/学习/思维类推文，可操作性标准放宽——提供**可操作的行动建议、思维框架、学习方法**即可视为高可执行性

#### 7.5.3 原创与深度 15 分 — 降低期望，但放宽标准

| 维度 | 分值 | 典型得分 |
|------|------|----------|
| 原创洞察 | 9 | 多数推文 **1-4 分**（原为 0-3 分）；Thread 可达 **5-7 分**（原为 5-7 分）；有观点有信息量即可获 **3-5 分** |
| 分析深度 | 6 | 多数推文 **2-3 分**（原为 1-2 分）；Thread 可达 **3-5 分** |

**注意**：
- 推文受字数限制，此维度大多数情况应给 **3-6 分**（原为 2-6 分）
- **放宽原创洞察标准**：不一定要有「深度 Thread」，**有独立思考、有观点、有信息整合价值**即可获 3-5 分
- 仅 Thread 或带长图/文档的推文可适当提高

#### 7.5.4 影响力 10 分

- 发布者行业地位、内容潜在传播价值、对行业趋势的影响
- 互动数据（点赞/转发）仅作次要参考，**不直接换算加分**
- **非技术类推文**（职场/学习/思维/生活）：考虑**长期价值**和**间接收益**，不应因「非技术」而降低影响力评分

#### 7.5.5 呈现质量 10 分

- 表达清晰度、信息组织、是否有误导性或模糊表述

### 7.6 减分项 -5 到 -25（原为 -5 到 -30）

| 类型 | 特征 | 减分 |
|------|------|------|
| 纯转发/无评论分享 | 只有链接或简单表情 | **-8 到 -15**（原为 -10 到 -20） |
...
| 领域偏离 | 纯政治、纯金融预测等与技术无关 | **-5 到 -10**（原为 -10 到 -15） |

**调整说明**：
- 整体降低减分幅度，让更多推文能达到 75+
- **明确排除**：职场/学习/思维/生活类推文**不适用「领域偏离」减分**
...
- **是否至少具备两项**：信息价值高 + 原创洞察 + 明显影响力？（90+ 放宽为"三者有其二"）
- 来源是否为 HIGH？MEDIUM 来源需极特殊理由，上限 93
...
1. 先识别来源优先级（从 `<ResourceSource>` 的 `<priority>` 获取），确定天花板和保底分
2. 识别推文类型，参考类型基准分
3. 按 5 维度逐项评分，在 remark 中列出各维度得分
4. 应用减分项（如适用）
5. 执行校准检查，必要时调整分数
6. 确保最终分数不超过天花板，且不低于保底分（除非有明显质量问题）

**核心调整**：
- **适度放宽 85+ 门槛**，让更多优质内容进入推荐区间（目标 ~25%）
- **非技术类推文**（职场/学习/思维/生活）如果质量优秀，可获 85+ 分
- **HIGH 来源常规内容保底 82 分，MEDIUM 来源保底 78 分**
- **头部厂商**（OpenAI/Anthropic/Google 等）的常规更新、热点资讯可适当提高分数
- **90+ 放宽为"三者有其二"**：信息价值高 + 原创洞察 + 明显影响力，满足两项即可

**默认姿态：适度放宽，让更多优质内容进入 85+ 区间。宁可偏高 2-3 分，避免过度压低。**
