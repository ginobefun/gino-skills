...
from scripts.shared.bestblogs_client import BestBlogsAdminClient, verify_saved_podcast_content
from scripts.shared.example_output import ExampleArgumentParser, print_failure, print_result


def parse_args() -> object:
    parser = ExampleArgumentParser(description="Save corrected podcast content and verify by reading it back.")
...
def main() -> int:
    action = "bestblogs.save_podcast_content"
    try:
        args = parse_args()
        payload = json.loads(Path(args.json_path).read_text("utf-8"))
        admin_client = BestBlogsAdminClient()
        write_payload, verify_payload = verify_saved_podcast_content(
            admin_client,
        ...
        print_failure(
            ...
            meta={"jsonPath": getattr(current_args, "json_path", None)},
        )
        return 1
