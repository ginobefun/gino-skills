...
description: "Use when 用户想审查 BestBlogs 待处理评分、纠正是否合格的判断，并从等待人工复核的内容里产出阅读清单。"
---
...
- 用户要 review 已评分但仍待人工判断的 BestBlogs 内容
- 用户要纠正 qualified / not qualified 判断，或产出一份值得深读的 shortlist
- 用户要学习近期评分偏好，再据此做人工校准

## When Not to Use

- 想直接抓取内容列表或正文时，使用 `bestblogs-fetcher`
- 想为单篇精选内容写推荐语时，使用 `bestblogs-article-recommender`
- 想处理 WAIT_ANALYSIS / WAIT_TRANSLATE 队列时，使用 `bestblogs-process-*` 和对应 worker skills

## Gotchas

- `markNotQualified` 是写操作，必须在输出 review 表格后等待用户确认
- 列表里会混入 `processFlowStatus == "CANCELLED"` 的内容，必须先过滤
- 来源优先级、评分天花板和减分项比单纯看总分更重要，不能只按 score 机械判断
- 推荐阅读清单和分数调整建议是两个输出，不要把“值得深读”和“需要下调”混成一类

## Related Skills

- `bestblogs-fetcher`：读取原始内容和元数据
- `bestblogs-article-recommender`：为最终保留的内容生成推荐语
- `read-deeply`：对 shortlist 中的重点内容做深读
- `bestblogs-process-*`：处理尚未完成分析或翻译的队列项
...
4. **内容质量信号**:
   - 标题是否清晰、`description` 是否有实质内容
   - `wordCount`/`readTime` 是否与评分匹配（短文高分需警惕）
   - `sourceName` 的历史质量和可信度

5. **时效与领域价值**:
   - `publishDate` 与当前时间的关系
   - 在其所属 `mainDomain` 中的价值

6. **推特特有检查**（仅推特类型）:
   - 极短推文（<50 字）不应超过 82 分，除非是官方首发里程碑
...
- ⬆️ 评分偏低（上调候选）：HIGH 来源评分 <80 的行业重要事件、HIGH/MEDIUM 来源的深度内容被低估、独特视角被忽略、优质策展类被当作低质聚合
- ⬇️ 评分偏高（下调候选）：LOW 来源评分 90+、编译转述类评分超过 89、标题党高分、纯新闻聚合高分、营销内容高分、同质化内容扎堆高分；**推特专项**：新闻转述 LOW 来源评分 >78、同事件重复转述评分未递减、极短推文（<50 字）评分 >82、领域偏离（政治/金融预测）评分 >75
...
```bash
python3 scripts/examples/bestblogs_mark_not_qualified.py \
  --id RAW_xxx \
  --adjust-score -3
```
