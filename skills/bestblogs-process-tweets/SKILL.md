...
description: "Use when 用户想批量分析 BestBlogs 待处理推文，通常按作者分组，并保存结构化评分、摘要和标签。"
---
...
- 用户要处理 BestBlogs 中等待分析的推文队列，并且任务天然按作者分组
- 用户希望一次分析同一作者的多条推文，再批量保存结果
- 请求重点是 tweet analysis queue，而不是公开视频、播客或文章

## When Not to Use

- 只想 fetch 原始推文或详情时，使用 `bestblogs-fetcher`
- 想处理文章、播客或视频队列时，分别使用对应 `bestblogs-process-*` orchestrator
- 想做翻译阶段处理时，优先使用 `bestblogs-translate-analysis-result`

## Gotchas

- 这是按作者批量处理的 skill，不要把不同作者的推文混进同一个分析输入
- 保存前必须校验分析结果中的 `tweetId` 和输入列表完全匹配
- 仍然需要串行按作者执行，避免不同作者的结果串写
- 写入阶段只看 HTTP 200 不够，必须检查响应体里的成功标记

## Related Skills

- `bestblogs-fetch-pending-content`：先拉出待分析推文队列
- `bestblogs-analyze-content`：内容级分析 worker，适合被 orchestrator 调用
- `bestblogs-fetcher`：读取公开内容而不是后台待处理队列
- `bestblogs-content-reviewer`：处理已经评分但待人工 review 的内容
...
```bash
python3 scripts/examples/bestblogs_save_tweet_analysis.py \
  --source-id SOURCE_xxx \
  --json /tmp/bestblogs-tweet-analysis-SOURCE_xxx.json
```
