...
- **D+G 合并（所有视频）**：所有视频单次 LLM 调用同时生成分析 JSON 和翻译 JSON，避免二次翻译往返损耗。语言方向由 resource `language` 字段决定，与文字稿语言无关（当前 Gem 统一输出中文文字稿）
...
**严格串行处理。** 每个视频按状态执行对应步骤（所有语言统一路径）：

| 状态 | 步骤 |
|------|------|
| WAIT_PREPARE | A → B → C → **D+G**（合并）→ E → [≥阈值] H |
| WAIT_ANALYSIS | **D+G**（合并）→ E → [≥阈值] H |

**D+G 合并的原因**：单次 LLM 调用同时产出分析结果和翻译结果，避免二次翻译的往返损耗，提升质量并减少一次调用。语言方向由 resource 的 `language` 字段决定（而非文字稿的文字语言，因为当前 Gem 统一输出中文文字稿）：

| resource.language | analysisResult 语言 | translateResult 语言 |
|-------------------|--------------------|--------------------|
| `zh_CN` | 中文 | 英文 |
| `en_US` | 英文 | 中文 |
...
### 步骤 D / D+G: 深度分析（英文视频同步翻译）

使用转录内容和视频元数据（阶段一列表），按照 `references/analysis_rubric.md` 中的评分体系进行深度分析。

**获取转录内容**：WAIT_PREPARE 使用步骤 B 本地文件；WAIT_ANALYSIS 通过以下方式获取（无需认证头，`success` 为**字符串类型**，使用 `markdown` 字段）：
...
4. **输出语言**：`remark` 始终中文，`domain`/`aiSubcategory` 用枚举值；其他内容字段按语言分支处理（见下）

**语言判断原则**：始终以 resource 的 `language` 字段为准，忽略文字稿语言（当前 Gem 统一输出中文）。

**单次调用同时输出两份 JSON**：
- `analysisResult`：内容字段使用 resource 原始语言（zh_CN → 中文，en_US → 英文）
- `translateResult`：内容字段使用目标翻译语言（zh_CN → 英文，en_US → 中文）
- `score`、`remark`、`domain`、`aiSubcategory` 只出现在 `analysisResult` 中，`translateResult` 不含这些字段
...
阶段三 D+G 已为所有视频同步生成翻译 JSON，**模式 A 直接执行步骤 H**（跳过 F 和 G）：
...
> **模式 A 跳过此步骤**：D+G 已为所有视频产出翻译 JSON，直接进入步骤 H。

**模式 B**（独立翻译 WAIT_TRANSLATE 队列）：使用 `/dify/resource/markdown` 端点获取：
...
> **模式 A 跳过此步骤**：翻译在 D+G 阶段已完成。

**模式 B 语言方向**：使用 `/dify/resource/markdown` 返回的 `languageName` 和 `destLanguageName`，或根据 resource `language` 字段判断（`zh_CN` → 英文，`en_US` → 中文）。
