...
| `/dify/resource/markdown` | GET | 读取 | 获取文章正文和已分析的结果 |
...
使用 `WAIT_TRANSLATE` 查询本周内等待翻译的文章：
...
- `timeFilter: "1w"` 表示查询最近 1 周（本周）的文章
- `pageSize: 20` 每页 20 条，若 `totalCount > 20`，自动翻页拉取全部
- `flowStatusFilter: "WAIT_TRANSLATE"` 仅查询等待翻译状态的文章
...
2. `saveTranslateResult` 必须使用与获取内容相同的 `{id}`
3. **禁止**将一篇文章的翻译结果保存到另一篇文章的 ID 上
4. **禁止**并行处理多篇文章

### 步骤 A: 获取文章内容和分析结果

使用 `/dify/resource/markdown` 端点获取文章正文和已分析的结果：
...
| 字段 | 说明 |
|------|------|
| `success` | `"true"` / `"false"`（字符串类型） |
| `title` | 文章标题 |
| `url` | 原文链接 |
| `sourceName` | 来源名称 |
| `priority` | 来源优先级 |
| `languageName` | 原始语言名称（如"中文"、"English"） |
| `destLanguageName` | 目标翻译语言名称（如"English"、"中文"） |
| `wordCount` | 字数 |
| `markdown` | 文章 Markdown 正文 |
| `analysisResult` | 已分析的结构化结果（**JSON 字符串**，需解析） |

**`analysisResult` 解析后的 JSON 结构**：
...
- `analysisResult` 中的内容字段已根据文章原始语言返回对应语言版本（中文文章返回中文分析，英文文章返回英文分析）
- 翻译方向由 `languageName`（原始语言）和 `destLanguageName`（目标语言）确定

若 `success` 为 `"false"` 或 `analysisResult` 为空，说明文章内容不可用或尚未分析，在进度中标注 ❌，**跳过该篇**，继续下一篇。

### 步骤 B: 翻译分析结果

**语言方向判断**：
- 直接使用步骤 A 返回的 `languageName` 和 `destLanguageName` 字段
- 示例：`languageName: "中文"`, `destLanguageName: "English"` → 中文翻译为英文
- 示例：`languageName: "English"`, `destLanguageName: "中文"` → 英文翻译为中文
...
1. **术语处理**
   ...
   - 禁止写成「人工智能（AI）」「AI Agent（AI 智能体）」「LLM（大语言模型）」等冗余形式
...
3. **格式要求**
   - 中文与英文、数字之间添加空格
   - 可适当使用基础 Markdown 语法（**强调**、`代码`），但不要过多使用，避免影响阅读流畅性
...
**端点**：`POST /api/admin/article/saveTranslateResult?id={id}`，id 为 query 参数。
...
| markdown 端点 `success: "false"` | 文章内容不可用 | 记录失败，跳过该文章，继续下一篇 |
| `analysisResult` 为空 | 文章尚未分析 | 记录失败，跳过该文章，继续下一篇 |
