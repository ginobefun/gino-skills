---
name: bili-actions
description: "通过 bilibili-cli 在B站执行操作 — 点赞、投币、三连、发布动态、删除动态、取关。适用场景：(1) 给视频点赞/投币/三连，(2) 发布B站动态，(3) 删除动态，(4) 取关UP主。触发短语：'B站点赞', 'B站投币', 'B站三连', '一键三连', '发B站动态', '删除动态', 'bili like', 'bili coin', 'bili triple', 'B站取关', '给视频投币', '发布动态', 'bilibili action', 'bilibili post', 'B站互动'。"
---
...
```bash
# 点赞
bili like BV1xxxxxxxxx --yaml

# 投币
bili coin BV1xxxxxxxxx --yaml

# 一键三连（点赞+投币+收藏）
bili triple BV1xxxxxxxxx --yaml
```
