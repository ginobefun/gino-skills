...
description: "Use when 用户想分析已发布内容的表现，尤其是 X 帖子的表现，并生成周度或月度的内容策略洞察。"
---
...
- 优先把这个 worker 作为阶段一主入口，再在本 skill 内完成时间窗口过滤、对比分析与策略总结
- 如需更细的 XGo 读取能力，复用 `scripts/shared/xgo_client.py`

## Shared Runtime

- 可复用中间结果优先写入 stable state，而不是散落在临时工作区
- 周报、月报或阶段性分析结果可由上游编排器决定是否同步到 `contents/tmp/workspace/YYYY-MM-DD/`
- 本 skill 负责分析，不负责直接执行平台写操作
...
```bash
python3 scripts/examples/xgo_content_analytics_source.py \
  --page-size 100 \
  --max-pages 2 \
  --tweet-type ALL
```
...
注意：worker 默认同样不传 `timeRange`，而是获取原始分页结果后按 `createdAt` 过滤。这样可同时获取本期和上期数据用于对比。若推文总量超过 200 条（2 页），可提高 `--max-pages`。
