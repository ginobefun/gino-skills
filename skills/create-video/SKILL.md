...
配音脚本遵循 `create-podcast/references/script_templates.md` 中的风格指南，核心要点:

- **内容驱动结构**: 由内容本身决定怎么讲，不要套死模板
- **语气**: 自然、平实、像和朋友聊天
- **信息准确**: 核心信息不失真，该说清楚的说清楚
- **禁止**: 恐惧渲染、宏大空泛、说教语气、引号、破折号、括号
- 视频配音比播客更简洁，视觉元素承担了部分信息传递
...
```json
{
  "platformMeta": {
    "bilibili": {
      "title": "关键词前置标题（不超 80 字）",
      "description": "精讲条目 + 时间戳导航",
      "tags": ["AI", "技术", "编程"],
      "partition": "科技/野生技术协会"
    },
    "youtube": {
      "title": "English title or bilingual",
      "description": "Timestamps + key points",
      "tags": ["AI", "tech"]
    }
  }
}
...
支持的分发平台:
- **B站**: 展示适配标题/描述/标签/分区，用户确认后手动上传或通过 API
- **YouTube**: 展示英文标题/描述/标签
- **微信视频号**: 展示短标题
