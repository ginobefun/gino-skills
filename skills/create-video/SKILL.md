---
name: create-video
description: "Use when 用户想把文本或音频内容转换成带解说的视频，用于 Bilibili、YouTube 等渠道。"
---
...
视频侧的 BestBlogs 内容读取与风格画像读取，优先走这些 worker，而不是正文中的直接 `curl`。
...
完整策略详情（6 种策略 + 决策流程 + Weekly 说明）见 `references/narrative-strategies.md`。

**6 种策略概览**：标准精讲+速览、深度聚焦、主题串联、对比分析、趋势信号、实操拆解。根据内容特征选择，可混合使用。

**决策流程**：3+条同趋势→趋势信号；2条对比→对比分析；2-3条话题重叠→主题串联；1条远超→深度聚焦；教程类→实操拆解；否则→标准精讲+速览。
...
**每周周刊/单篇文章/任意内容**: 同 `create-podcast` 的 worker-first 内容获取方式。
...
脚本必须优先读取 `${CLAUDE_PLUGIN_DATA}/gino-skills/manage-daily-content/memory/style-profile.md`，仅在缺失时临时兼容旧路径 `contents/style-profile.md`，注入个人声音特征。
