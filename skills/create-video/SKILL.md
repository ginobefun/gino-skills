...
好的视频不是孤立的。查阅近期历史内容，发现关联和延续：

```bash
# 查看最近 7 天的早报/周刊 + 最近的视频产出
ls contents/bestblogs-digest/ | tail -7
ls contents/video/daily/ | tail -5
```

**五类关联**: 话题延续（上次聊过 XX，有新进展）、趋势积累（连续多天同话题）、观点演变（之前预测现在验证）、同主题不同视角（综合多源信息）、避免重复（上期已覆盖的只讲增量）。

**跨来源综合**: 多条内容涉及同一话题但来自不同来源时，要综合视角而非逐条孤立讲述，这是串联策略的天然素材。
