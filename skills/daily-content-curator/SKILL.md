...
- **评分阈值需动态调整**：初始阈值过高会导致选题数量不足（如 <20 个），应根据实际数据分布调整，确保最终输出 30 个选题
- **时间范围**：默认查询最近 3 天数据，确保选题充足
...
读取最近 3 天的 `contents/daily-curation/YYYY-MM-DD/curation.md`，提取已推荐内容的 URL 构建去重集合。详细实现见 `references/api_reference.md`。}
...
默认查询最近 **3 天**数据，确保选题池充足。所有内容按 `publishTimeStamp` 或 `createdAt` 过滤，只保留最近 72 小时内发布的。

时间范围参数：
- BestBlogs: `timeFilter: "3d"`
- XGo: `timeRange: "WEEK"`（配合客户端过滤最近3天）
...
**响应数据结构**：
- BestBlogs: `response.dataList` (数组) 或 `response.data.dataList` (包装结构)
- XGo: `response.data.data` (数组)，非 `data.list`

**客户端过滤**：BestBlogs 保留 `score >= 75`，XGo 保留 `influenceScore >= 40`。若扩展到 3 天，按 `publishTimeStamp` 或 `createdAt` 过滤最近 72 小时。
...
**空值处理**：所有字段可能为 null，使用 `item.get('field') or default` 模式安全取值。详见 `references/api_reference.md`。

### 2.2 去重

1. **URL 去重**: 同 URL 保留评分高者
2. **跨源推文去重**: 同一推文保留 BestBlogs 版本（有 AI 摘要）
3. **历史去重**: 必须排除最近 3 天已推荐的相同 URL（提取历史文件中的 URL 加入去重集合）

详细实现见 `references/api_reference.md` 的去重代码示例。
...
**阈值调整策略**：若某分类候选数量不足目标数量的 80%，应降低该分类的分数阈值：
- 深度不足：从 >=65 降到 >=55
- 互动不足：从 >=55 降到 >=48
- 快评不足：从 >=50 降到 >=40

确保最终输出接近 30 个选题（10+12+8）。
...
| 用户表述 | 调整 |
|---------|------|
| "早间" / "早上" | BestBlogs `timeFilter: "3d"` + XGo `WEEK`，客户端过滤最近24小时 |
| "晚间" / "下午" | 客户端过滤近 8 小时 |
| "本周" / "这周" | BestBlogs `1w` + XGo `MONTH` |
