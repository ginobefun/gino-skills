...
从以上数据源提取风格特征后，优先通过 `scripts/examples/content_style_profile_state.py write --from-file ... --sync-legacy` 写入 stable memory；旧路径 `contents/style-profile.md` 仅作临时兼容副本：
...
**更新频率**: 每周重新生成一次，或用户要求时更新。若 stable memory 中的 `style-profile.md` 存在且为本周内生成，直接加载不重新分析。
