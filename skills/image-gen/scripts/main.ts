...
async function loadEnv(): Promise<void> {
  ...
  const homeEnv = await loadEnvFile(path.join(home, ".gino-skills", ".env"));
  const cwdEnv = await loadEnvFile(path.join(cwd, ".gino-skills", ".env"));
...
function detectProvider(args: CliArgs): Provider {
  ...
  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, DASHSCOPE_API_KEY, or REPLICATE_API_TOKEN.\n" +
      "Create ~/.gino-skills/.env or <cwd>/.gino-skills/.env with your keys."
  );
}
