...
function loadConfig(): WechatConfig {
  const cwdEnvPath = path.join(process.cwd(), ".gino-skills", ".env");
  const homeEnvPath = path.join(os.homedir(), ".gino-skills", ".env");
  ...
  if (!appId || !appSecret) {
    throw new Error(
      ...
      "Set via environment variables or in .gino-skills/.env file."
    );
  }
