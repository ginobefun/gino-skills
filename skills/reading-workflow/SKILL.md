...
description: "每日阅读工作流 — 基于阅读清单逐篇引导阅读、深度分析、思考记录和素材汇总。适用场景: (1) 开始今天的阅读, (2) 逐篇引导式阅读, (3) 记录阅读思考和观点, (4) 汇总阅读素材用于内容创作, (5) 对文章做快速或深度分析, (6) 建立每日阅读习惯。触发短语: '开始阅读', '今天的阅读', '继续阅读', '阅读工作流', '逐篇阅读', 'start reading', 'reading workflow', 'continue reading', '帮我读', '阅读引导', '边读边想', '阅读笔记', '今天读什么', '读完了，下一篇', 'next article', '阅读素材汇总'"
---
...
本 skill 不直接调用 API。文章内容获取通过以下方式:
- **BestBlogs 文章**: 通过 `bestblogs-fetcher` 的端点获取 Markdown 正文（需要 `BESTBLOGS_API_KEY`）
- **网页文章**: 通过 Jina Reader（`https://r.jina.ai/{URL}`）获取，无需认证
- **推文**: 从 curator 输出中直接获取完整文本

深度分析模式调用 `deep-reading` skill，无额外认证需求。
...
`materials.md` 使用结构化格式，方便 content-synthesizer 直接解析。
