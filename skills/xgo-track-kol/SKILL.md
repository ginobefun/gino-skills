---
name: xgo-track-kol
description: "Use when 用户想对一个或两个 X 账号做深度画像或对比分析，包括活跃度、内容构成、互动表现和代表性推文。"
---
...
- 用户要做单个账号的深度画像，或对比两个账号的内容与影响力
- 用户需要 activity、content mix、engagement 和高光推文的完整分析报告
- 任务重点是“分析”和“比较”，而不是简单查看资料

## When Not to Use

- 只想快速查看某个账号资料和最新推文时，使用 `xgo-view-profile`
- 想搜索某个话题下的推文时，使用 `xgo-search-tweets`
- 想管理 follows 或 lists 时，使用 `xgo-manage-follows`、`xgo-manage-lists`

## Gotchas

- 双账号对比模式要保证两边拉取的是同构数据，不能一边 recent 一边 latest
- 高互动不等于稳定影响力，报告里要区分“爆款”和“稳定输出”
- 关注状态只是附带维度，不要把它写成分析结论本身
- 对比报告必须先做各自独立分析，再做横向比较，避免直接拼指标表

## Related Skills

- `xgo-view-profile`：轻量资料查看
- `xgo-search-tweets`：按话题抓推文样本
- `xgo-manage-follows`：跟进 follow 状态和推荐关注
- `xgo-organize-follows`：把分析后值得保留的账号整理进列表体系

## Shared Scripts

- `scripts/examples/xgo_user_activity.py`：抓取单个账号的 recent / influence 推文样本
- `scripts/examples/xgo_following_status.py`：读取当前账号对目标用户的 follow 状态
- `scripts/shared/xgo_client.py`：统一处理认证、`success:false`、分页和结构化返回

## Worker Entrypoints

优先使用 worker，而不是直接在主流程里手写多个 `curl`：
...
若是双账号对比，对两个用户分别运行同样的 worker，再做横向比较。
...
`xgo_user_activity.py` 会统一完成：
- 实时用户画像
- 近期推文样本（`sortType=recent`）
- 高影响力推文样本（`sortType=influence`）

`xgo_following_status.py` 会统一完成：
- 关注状态
- 已有标签和备注读取

**双用户对比模式**: 对第二个用户运行同样的 worker（共 4 次 worker 调用），不要在主流程里重新拼一套原始请求。

### Raw Endpoint Fallback

只有在 worker 不满足需求或需要排查底层响应时，才回退到 `references/api_reference.md` 里的原始端点。那份文档现在是实现参考，不是首选执行路径。

**速率说明**: 单用户 4 请求，双用户 8 请求，远低于 PLUS 200 次/分限制。
