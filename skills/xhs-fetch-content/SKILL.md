...
description: "Use when 用户想读取或搜索小红书内容，包括热门笔记、搜索结果、笔记详情、评论、话题或创作者资料。"
---
...
- `python3 scripts/examples/xhs_fetch_worker.py --subcommand search AI`
- `python3 scripts/examples/xhs_fetch_worker.py --subcommand read <note_id>`
- `python3 scripts/examples/xhs_fetch_worker.py --subcommand comments <note_id> --all`

这些入口统一返回 worker JSON，适合被内容筛选、内容分析等上游 skill 直接消费。

## Shared Runtime

- 读取类主路径优先走 `scripts/examples/xhs_fetch_worker.py`
- 如需缓存笔记详情或评论快照，由 orchestrator 决定写入 stable state 或 workspace
- 原始 `xhs ... --yaml` 命令保留作调试路径
...
python3 scripts/examples/xhs_fetch_worker.py --subcommand search "关键词" --sort popular
python3 scripts/examples/xhs_fetch_worker.py --subcommand search "关键词" --sort latest
```
...
```bash
python3 scripts/examples/xhs_fetch_worker.py --subcommand hot
python3 scripts/examples/xhs_fetch_worker.py --subcommand hot -c tech
python3 scripts/examples/xhs_fetch_worker.py --subcommand feed
```
...
```bash
python3 scripts/examples/xhs_fetch_worker.py --subcommand user <user_id>
python3 scripts/examples/xhs_fetch_worker.py --subcommand user-posts <user_id>
```
...
```bash
python3 scripts/examples/xhs_fetch_worker.py --subcommand topics "AI"
python3 scripts/examples/xhs_fetch_worker.py --subcommand search-user "用户名"
```
...
```bash
python3 scripts/examples/xhs_fetch_worker.py --subcommand favorites
python3 scripts/examples/xhs_fetch_worker.py --subcommand unread
python3 scripts/examples/xhs_fetch_worker.py --subcommand notifications
```
